...
req_api = requests.get('https://api.thecatapi.com/v1/images/search')
if req_api.status_code == 200:
    url_photo = req_api.json()[0]['url']
    print(url_photo)
    requests.get(f'{API_URL}{BOT_TOKEN}/sendPhoto?chat_id=1859997762&photo={url_photo}')
else:
    print(f'Код статуса - {req_api.status_code}')
